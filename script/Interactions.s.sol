...
//Chainlink VRF infrastructure setup
...
contract CreateSubscription is CodeConstants, Script {
    ...
    function createSubscription(address vrfCoordinatorV2_5, address account) public returns (uint256, address) {
        ...
        if (block.chainid == LOCAL_CHAIN_ID) {
            // Use mock for local network
            subId = VRFCoordinatorV2_5Mock(vrfCoordinatorV2_5).createSubscription();
        } else {
            // Use real interface for testnets/mainnet
            subId = IVRFSubscriptionV2Plus(vrfCoordinatorV2_5).createSubscription();
        }
        vm.stopBroadcast();
